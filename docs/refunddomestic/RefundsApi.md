...
[**CreateAbnormalRefund**](#createabnormalrefund) | **Post** /v3/refund/domestic/refunds/{refund_id}/apply-abnormal-refund | 发起异常退款
...
## CreateAbnormalRefund

> Refund CreateAbnormalRefund(CreateAbnormalRefundRequest)
...
func main() {
	...
	svc := refunddomestic.RefundsApiService{Client: client}
	resp, result, err := svc.CreateAbnormalRefund(ctx,
		refunddomestic.CreateAbnormalRefundRequest{
			RefundId:    core.String("50000000382019052709732678859"),
			SubMchid:    core.String("1900000109"),
			OutRefundNo: core.String("1217752501201407033233368018"),
			Type:        refunddomestic.TYPE_COUPON.Ptr(),
			BankType:    core.String("ICBC_DEBIT"),
			BankAccount: core.String("d+xT+MQCvrLHUVDWv/8MR/dB7TkXLVfSrUxMPZy6jWWYzpRrEEaYQE8ZRGYoeorwC+w=="),
			RealName:    core.String("UPgQcZSdq3zOayJwZ5XLrHY2dZU1W2Cd"),
		},
	)

	if err != nil {
		// 处理错误
		log.Printf("call CreateAbnormalRefund err:%s", err)
	} else {
		// 处理返回结果
		log.Printf("status=%d resp=%s", result.Response.StatusCode, resp)
	}
...
------------- | ------------- | -------------
**ctx** | **context.Context** | Golang 上下文，可用于日志、请求取消、请求跟踪等功能|
**req** | [**CreateAbnormalRefundRequest**](CreateAbnormalRefundRequest.md) | API `refunddomestic` 所定义的本接口需要的所有参数，包括`Path`/`Query`/`Body` 3类参数|
