...
type TransferBatchEntity struct {
	...
	TransferDetailList []TransferDetailCompact `json:"transfer_detail_list"`
	// 转账批次单基本信息
...
func (o TransferBatchEntity) MarshalJSON() ([]byte, error) {
	...
	if o.TransferDetailList == nil {
		return nil, fmt.Errorf("field `TransferDetailList` is required and must be specified in TransferBatchEntity")
	}
	toSerialize["transfer_detail_list"] = o.TransferDetailList
...
type TransferDetailInput struct {
	...
	// 商户appid下，某用户的openid
	Openid *string `json:"openid"`
	// 收款方姓名。采用标准RSA算法，公钥由微信侧提供 明细转账金额 >= 2,000时，该笔明细必须填写收款用户姓名 同一批次转账明细中的姓名字段传入规则需保持一致，也即全部填写、或全部不填写 若商户传入收款用户姓名，微信支付会校验用户openID与姓名是否一致，并提供电子回单
	UserName *string `json:"user_name,omitempty" encryption:"EM_APIV3"`
